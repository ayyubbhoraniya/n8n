...
/**
 * The one v1.3 option v2 does not carry: the new library has no equivalent, so
 * it could only ever be a control that does nothing.
 */
const DROPPED_IN_V2 = ['autoCommitThreshold'];

/** v2 adds no options of its own; it only drops the one above. */
...
describe('KafkaTriggerV2 description', () => {
	...
	it('carries every v1.3 option except the one that controls nothing', () => {
